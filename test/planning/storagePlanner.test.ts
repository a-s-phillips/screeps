...
import { findStorageSite } from "../../src/planning/storagePlanner";

const allWalkable = () => true;
const noneOccupied = () => false;

describe("findStorageSite", () => {
  it("finds the nearest free tile to the anchor", () => {
    const anchor = { x: 25, y: 25 };

    const result = findStorageSite(allWalkable, noneOccupied, anchor);
  ...
  it("skips tiles that are not walkable", () => {
    ...
    const result = findStorageSite(isWalkable, noneOccupied, anchor);
  ...
  it("skips tiles that are already occupied", () => {
    ...
    const result = findStorageSite(allWalkable, isOccupied, anchor);
  ...
  it("returns null when nothing is available within the search radius", () => {
    ...
    const result = findStorageSite(() => false, noneOccupied, anchor);
