...
import { findSpawnSite } from "../../src/planning/spawnPlanner";

const allWalkable = () => true;
const noneOccupied = () => false;

describe("findSpawnSite", () => {
  it("finds the nearest free tile to the anchor", () => {
    const anchor = { x: 25, y: 25 };

    const result = findSpawnSite(allWalkable, noneOccupied, anchor);
  ...
  it("skips tiles that are not walkable", () => {
    ...
    const result = findSpawnSite(isWalkable, noneOccupied, anchor);
  ...
  it("skips tiles that are already occupied", () => {
    ...
    const result = findSpawnSite(allWalkable, isOccupied, anchor);
  ...
  it("returns null when nothing is available within the search radius", () => {
    ...
    const result = findSpawnSite(() => false, noneOccupied, anchor);
