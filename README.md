...
Status: **early / post-MVP.** One room, four creep roles, automatic extension/container/road placement, container-based energy hauling, and a telemetry pipeline. No multi-room logic, no combat, no economy beyond "keep the controller fed."
...
- **Roles** (`src/roles/`) — harvester, upgrader, builder, hauler. All share a `working`/`!working` state machine (`src/roles/shared.ts`): gather energy until full, then spend it until empty. Harvesters transfer into an adjacent container when one's available, falling back to delivering straight to the spawn/extensions (`deliverEnergy`, shared with haulers) and then upgrading the controller; builders prefer open construction sites and fall back to upgrading; upgraders withdraw from the nearest container with energy (typically the one next to the controller) and fall back to solo-harvesting a source only if no container has any. Haulers gather via `withdrawFromNearestContainer` and, once the spawn/extensions are full, top off the controller-adjacent container before falling back to upgrading directly.
- **Room planning** (`src/planning/`) — once a room is visible, `roomPlanner.ts` places construction sites for three structure types, one per tick each: `STRUCTURE_EXTENSION`s up to the current RCL's cap (`extensionPlanner.ts` searches concentric rings out from the spawn for the nearest free, walkable, checkerboard-spaced tile), one `STRUCTURE_CONTAINER` next to each source and next to the controller, whichever doesn't have one yet (`containerPlanner.ts`, sharing the room's overall container cap for that RCL), and `STRUCTURE_ROAD`s along the spawn↔source and spawn↔controller routes (`roadPlanner.ts`, using `Room.findPath` to compute the tile list once and caching it in `RoomMemory.roadPlan` so pathfinding only runs once per room, not every tick). Extension/container siting share ring-search geometry from `src/utils/grid.ts`.
